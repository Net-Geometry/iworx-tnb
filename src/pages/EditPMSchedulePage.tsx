 ...
 * EditPMSchedulePage
 * 
 * This page provides a dedicated interface for editing existing preventive maintenance schedules.
 * Users can modify schedule parameters, update assignments, and adjust configurations.
 * 
 * Features:
...
import { PMScheduleInsert, usePMSchedule, useUpdatePMSchedule } from "@/hooks/usePMSchedules";
...
const EditPMSchedulePage = () => {
  const navigate = useNavigate();
  const { id } = useParams<{ id: string }>();
  const { currentOrganization } = useAuth();
  
  // Data hooks
  const { data: schedule, isLoading: scheduleLoading } = usePMSchedule(id!);
  ...
  // ============================================================================
  // Effect: Populate form with existing schedule data
  // ============================================================================
  ...
  const onSubmit = async (values: PMScheduleFormValues) => {
    try {
      ...
      await updatePMSchedule.mutateAsync({ id: id!, updates: scheduleData });
    ...
    return (
      <div className="container mx-auto py-6">
        <div className="text-center">
          ...
          <p className="text-muted-foreground mt-2">The requested schedule could not be found.</p>
          <Button onClick={() => navigate("/preventive-maintenance")} className="mt-4">
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold">Edit PM Schedule</h1>
          <p className="text-muted-foreground">
            Update preventive maintenance schedule configuration
          </p>
        </div>
      </div>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="flex justify-end gap-4">
            ...
            <Button
              type="submit"
              disabled={updatePMSchedule.isPending}
            >
              {updatePMSchedule.isPending ? "Updating..." : "Update PM Schedule"}
            </Button>
          </div>
        </form>
      </Form>
...
export default EditPMSchedulePage;
