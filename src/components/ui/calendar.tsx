...
function Calendar({ className, classNames, showOutsideDays = true, renderDayContent, ...props }: CalendarProps) {
  // Create custom DayContent component if renderDayContent is provided
  const CustomDayContent = renderDayContent 
    ? (dayProps: DayContentProps) => <>{renderDayContent(dayProps)}</>
    : undefined;
  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        cell: "h-12 w-12 text-center text-sm p-0 relative [&:has([aria-selected].day-range-end)]:rounded-r-md [&:has([aria-selected].day-outside)]:bg-accent/50 [&:has([aria-selected])]:bg-accent first:[&:has([aria-selected])]:rounded-l-md last:[&:has([aria-selected])]:rounded-r-md focus-within:relative focus-within:z-20 hover:bg-accent/50 transition-colors",
        day: cn(buttonVariants({ variant: "ghost" }), "h-12 w-12 p-0 font-normal aria-selected:opacity-100 hover:bg-accent/50 transition-colors"),
      ...
      components={{
        ...
        ...(CustomDayContent && { DayContent: CustomDayContent }),
      }}
      {...props}
    />
  );
