  ...
  try {
    ...
    if (method === "GET" && pathParts[0] === "stats") {
      ...
      const [active, overdue, dueThisWeek, completedThisMonth] = await Promise.all([
        supabase.schema("pm_service").from("schedules").select("id", { count: "exact", head: true })
    ...
    if (method === "POST" && pathParts[1] === "generate-work-order") {
      ...
      await supabase.schema("pm_service").from("schedules")
        .update({ 
    ...
    if (method === "POST" && pathParts.length === 0) {
      ...
      const { count } = await supabase.schema("pm_service").from("schedules")
